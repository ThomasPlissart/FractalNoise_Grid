...
class FractalGrid {
  ...
  int mode;
  float posX, posY, xRot = 45, yRot = 0; //transformations dans l'espace
  ...
  void calculateNoise() {
    float mx = -map(mouseX,0,width,0,50), 
    ...
    if (opacity > 0) { // permet de ne pas calculer tout ça si on ne le voit pas -> optimisation performances
      ...
      if (mode == 1) {
        for (int y = 0; y < rows-1; y++) {
          beginShape(POINTS);  
          stroke(255, map(y, 0, rows-1, fadeOut, fadeIn)*opacity);
          for (int x = 0; x < cols; x++) {  
